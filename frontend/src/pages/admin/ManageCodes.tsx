...
import {
    getConcoursCodes, createConcoursCode,
    getValidationCodes, createValidationCode,
    Filiere, Annee, OptionLIC,
} from '../../lib/store';
...
const ManageCodes: React.FC = () => {
    ...
    const [concoursCodes, setConcours]  = useState(getConcoursCodes());
    const [validationCodes, setValid]   = useState(getValidationCodes());
    ...
    const refreshConcours  = () => setConcours(getConcoursCodes());
    const refreshValidation = () => setValid(getValidationCodes());
    ...
    const handleCreateConcours = (e: React.FormEvent) => {
        ...
        createConcoursCode({
            nom_complet: cNom,
            filiere:     cFiliere as Filiere,
            annee:       cAnnee  as Annee,
            option:      needsOption ? (cOption as OptionLIC) : undefined,
        });
        refreshConcours();
        closeModal();
    };
        ...
        <DashboardLayout>
            ...
            <div className="mc-hero">
                <div className="mc-hero-text">
                    ...
                    <div className="mc-hero-badges">
                        <span className="mc-hero-badge">
                            <IonIcon icon={keyOutline} />
                            {concoursTotal} codes concours
                        </span>
                        <span className="mc-hero-badge">
                            <IonIcon icon={shieldCheckmarkOutline} />
                            {concoursDispo} disponibles
                        </span>
                        <span className="mc-hero-badge">
                            <IonIcon icon={lockClosedOutline} />
                            {validDispo} externes valides
                        </span>
                    </div>
                </div>
                <div className="mc-hero-action">
                    {tab === 'concours' ? (
                        <IonButton className="mc-hero-btn" fill="outline" onClick={() => setModalOpen(true)}>
                            ...
                            Nouveau code concours
                        </IonButton>
                    ) : (
                        <IonButton className="mc-hero-btn" fill="outline" onClick={handleCreateValidation}>
                            <IonIcon slot="start" icon={addCircleOutline} />
            ...
            <div className="mc-stats-row">
                <div className="mc-stat-chip">
                    <IonIcon icon={keyOutline} className="mc-stat-chip-icon" />
                    <span className="mc-stat-chip-count">{concoursDispo}/{concoursTotal}</span>
                    <span className="mc-stat-chip-label">Concours dispos</span>
                </div>
                <div className="mc-stat-chip">
                    <IonIcon icon={lockClosedOutline} className="mc-stat-chip-icon" />
                    <span className="mc-stat-chip-count">{validDispo}/{validTotal}</span>
                    <span className="mc-stat-chip-label">Externes valides</span>
                </div>
                <div className="mc-stat-chip">
                    <IonIcon icon={statsChartOutline} className="mc-stat-chip-icon" />
                    <span className="mc-stat-chip-count">{concoursCodes.filter(c => c.used).length}</span>
                    <span className="mc-stat-chip-label">Concours utilisés</span>
                </div>
                <div className="mc-stat-chip">
                    <IonIcon icon={timeOutline} className="mc-stat-chip-icon" />
                    <span className="mc-stat-chip-count">
                        {validationCodes.filter(c => !c.used && new Date(c.expires_at) < new Date()).length}
                    </span>
                    <span className="mc-stat-chip-label">Externes expirés</span>
                </div>
            </div>
            ...
            <div className="mc-toolbar">
                ...
                >
                    <IonSegmentButton value="concours" className="mc-seg-btn">
                        <IonLabel>Codes Concours</IonLabel>
                    </IonSegmentButton>
                    <IonSegmentButton value="validation" className="mc-seg-btn">
                        <IonLabel>Codes Validation</IonLabel>
                    </IonSegmentButton>
                </IonSegment>
            </div>
                        ...
                        <CardContent padding="sm">
                            <div className="mc-table-scroll">
                                <table className="mc-table">
                                    <thead>
                                        <tr className="mc-thead-tr">
                                            <th className="mc-th">Code</th>
                                            <th className="mc-th">Étudiant</th>
                                            <th className="mc-th">Filière</th>
                                            <th className="mc-th">Année</th>
                                            <th className="mc-th">Statut</th>
                                            <th className="mc-th mc-th--actions">Copier</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {concoursCodes.length === 0 ? (
                                            <tr>
                                                <td colSpan={6} className="mc-empty-row">
                                                    <IonIcon icon={keyOutline} className="mc-empty-icon" />
                                                    <span>Aucun code généré</span>
                                                </td>
                                            </tr>
                                        ) : concoursCodes.map(c => (
                                            <tr key={c.id} className="mc-tr">
                                                <td className="mc-td mc-td--code">
                                                    <span className="mc-code">{c.code}</span>
                                                </td>
                                                <td className="mc-td mc-td--name">{c.nom_complet}</td>
                                                <td className="mc-td">
                                                    <Badge variant="info" size="sm">{c.filiere}</Badge>
                                                </td>
                                                <td className="mc-td">
                                                    <span className="mc-annee">{c.annee}{c.option ? ` (${c.option})` : ''}</span>
                                                </td>
                                                <td className="mc-td">
                            ...
                            <div className="mc-table-scroll">
                                <table className="mc-table">
                                    <thead>
                                        <tr className="mc-thead-tr">
                                            <th className="mc-th">Code</th>
                                            <th className="mc-th">Expire le</th>
                                            <th className="mc-th">Statut</th>
                                            <th className="mc-th mc-th--actions">Copier</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {validationCodes.length === 0 ? (
                                            <tr>
                                                <td colSpan={4} className="mc-empty-row">
                                                    <IonIcon icon={lockClosedOutline} className="mc-empty-icon" />
                                                    <span>Aucun code généré</span>
                                                </td>
                                            </tr>
                        ...
                        <div className="mc-form-section">
                            ...
                            <div className="mc-field mc-field--full">
                                <label className="mc-field-label">
                                    <IonIcon icon={personOutline} className="mc-field-icon" />
                                    Nom complet <span className="mc-required">*</span>
                                </label>
                                <IonInput
