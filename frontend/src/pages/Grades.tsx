...
const StudentGradesView: React.FC = () => {
    ...
    const { data: allEntries = [], isLoading } = useQuery<any[]>({ queryKey: ['grades', 'student', user.id],
...
const CourseGradeView: React.FC<CourseGradeViewProps> = ({ course, onBack }) => {
    ...
    const { data: students = [] } = useQuery<any[]>({ queryKey: ['users', 'students'],
        queryFn: () => courseService.list().then(() => fetch('/api/users?role=etudiant_concours').then(r => r.json())),
        // simpler: use userService directly
        enabled: false,
    });

    const { data: entries = [], isLoading } = useQuery<any[]>({ queryKey: ['grades', 'course', course.id],
    ...
    const unpublishMutation = useMutation<any,any,any>({
...
const ProfessorGradesView: React.FC = () => {
    ...
    const { data: courses = [], isLoading } = useQuery<any[]>({ queryKey: ['courses', 'professor'],
